...
12. Write .env.local + push to Vercel:
    ...
    INBOUND_EMAIL_DOMAIN            ← e.g. in.{domain} (default in code)
...
- **CloudMailin**: Account creation and address target configuration are dashboard-only (cloudmailin.com — no account creation API). Once the account exists, the only automatable parts are: (1) the MX record for `in.{domain}` via Vercel DNS API, (2) `CLOUDMAILIN_WEBHOOK_SECRET` generation + push to Vercel. The shared secret is *generated by you* (not given by CloudMailin) — you generate it, push it to Vercel, and paste it into the CloudMailin webhook URL. MX target is always `mx.cloudmailin.net.` (priority 10) — confirmed as universal across accounts/regions. Free tier: 10,000 messages/month, no credit card required.
