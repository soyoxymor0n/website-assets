...
| Vercel beta branch domain | 🟢 Automated (2026-07-15) | `POST /v10/projects/{id}/domains` with `gitBranch: "beta"` — CLI has no branch-scoping flag. **REQUIRED on every project** (blueprint `docs/env-isolation-instruction.md`), and the step most likely to be missed — not by this script, but by anyone who REUSES an existing Vercel project and therefore never runs the domain phase at all (FineAlly, 2026-08-29: apex + www were already attached from months earlier, so `beta.fineally.app` simply never got created). On house domains DNS needs nothing: Vercel runs the nameservers, so the subdomain verifies on attach. A 302 to Vercel SSO afterwards is correct, not a misconfiguration. |
...
UPSTASH_ACCOUNT_EMAIL=...                    # the email you log into console.upstash.com with — Management API auth is Basic email:key, not Bearer
...
- **Upstash Management API auth — Basic, not Bearer (fixed 2026-08-31)**: `scaffold.js` sent `Authorization: Bearer $UPSTASH_MANAGEMENT_API_KEY` for both the `upstash-redis` and `upstash-qstash` steps. That 401s with `"token contains an invalid number of segments"` — Upstash tries to parse a Bearer value as a JWT — which reads exactly like a stale/malformed key even against a perfectly valid one (reproduced live against an active, non-expiring key). The Management API actually wants HTTP **Basic** auth as `email:api_key` (base64 of `"${UPSTASH_ACCOUNT_EMAIL}:${UPSTASH_MANAGEMENT_API_KEY}"`), confirmed live 2026-08-31 (200, correct DB list) with the exact key that 401'd as Bearer. Fixed via the shared `upstashAuthHeader()` helper; `UPSTASH_ACCOUNT_EMAIL` is a new required secret alongside the key.
- **Upstash Redis**: `POST /v2/redis/database` works — correct fields are `database_name` (not `name`), `platform` (aws/gcp, required), `primary_region`. Free tier = 1 DB max — the `upstash-redis` step now falls back to `GET /v2/redis/databases` and reuses an existing DB (matched by name, else the first one) when create fails, rather than just erroring out.
- **Upstash QStash token**: `QSTASH_TOKEN` must be copied manually from console.upstash.com/qstash (no Management API endpoint). Once you have it, signing keys are automated: `GET https://qstash.upstash.io/v2/keys` with the same Basic auth as above.
