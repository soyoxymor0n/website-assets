...
if (step("clerk", "Clerk → create application & configure")) {
  manual(
    ...
    [
      ...
      "Only once the application exists (you create it by hand above — nothing here can do that for you), go to: Configure → Customization → Avatars → Default user avatar, on BOTH the dev and prod instance:",
      "  Background: Solid → the app's primary colour as HEX (src/index.css --primary, converted from OKLCH)",
      "  Foreground: Initials → the app's --primary-foreground as HEX; must be ≥4.5:1 against the background, else use #FFFFFF / #111111",
...
const manualItems = [
  ...
  `☐ Clerk: Default user avatar — Solid background in the app's primary colour, Initials foreground in --primary-foreground (≥4.5:1), dev AND prod instance`,
